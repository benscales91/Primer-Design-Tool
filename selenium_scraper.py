...
class PrimerBankPipeline:
    ...
    def scrape_details(self, pid, retry_count=0):
        ...
        # THE ERROR CATCHER
            ...
            # 1. Check if it's a physical network/WiFi dropout
            ...
            if any(err in error_msg for err in network_errors):
                logger.error(
                    f"Network drop detected on {clean_pid}. Waiting 30 seconds for Windows to auto-reconnect...")

                if "10054" in error_msg or "ERR_CONNECTION_CLOSED" in error_msg:
                    logger.warning("Server forcibly closed connection. Likely an IP block. Rotating VPN...")
                    self.rotate_vpn()
                else:
                    # Otherwise, it's a standard Wi-Fi drop. Just wait for Windows.
                ...
                return self.scrape_details(clean_pid)  # Retry the ID!

            # 2. If it's a normal HTML/Parsing error, log it and save the partial data
            logger.error(f"Scrape failed for {clean_pid}: {e}. Saving partial data to CSV.")

            # Tag it so you can easily find it in Excel later
